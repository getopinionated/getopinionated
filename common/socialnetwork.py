...
def posttotwitter(message):
    ...
    except: #(ignore too general except) pylint: disable=W0702
        print "posting to twitter feed failed. Message was: ", message


# Note: This doesn't work yet, as it is impossible to keep keys for longer than 60 days, you can however connect your twitter-account to facebook
def posttofacebook(message):
    APP_ID = ""
    APP_SECRET = ""
    
    APP_TOKEN = urllib.urlopen(
        "https://graph.facebook.com/oauth/access_token?client_id=%s&client_secret=%s&grant_type=client_credentials" % (APP_ID, APP_SECRET)).read()
    APP_TOKEN = APP_TOKEN[13:]
    print APP_TOKEN
    #Step 1: ask permission
    #https://www.facebook.com/dialog/permissions.request
    #    ?_path=permissions.request
    #    &app_id=298186733641393
    #    &redirect_uri=https%3A%2F%2Fwww.facebook.com%2Fconnect%2Flogin_success.html%3Fdisplay%3Dpage
    #    &response_type=token
    #    &fbconnect=1
    #    &perms=status_update,publish_stream,manage_pages
    #    &from_login=1&m_sess=1&rcount=1
    #Step 2: get short lived access token
    #https://developers.facebook.com/tools/explorer

     # (ignore missing type GraphAPI) pylint: disable=E0602
    appgraph = GraphAPI(APP_TOKEN)
    
    SHORT_ACCESS_TOKEN = ""
    usergraph = GraphAPI(SHORT_ACCESS_TOKEN)
    print usergraph.request("/oauth/access_token", {
        "client_id":APP_ID,
